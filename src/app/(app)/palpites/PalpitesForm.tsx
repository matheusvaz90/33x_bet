...
export default function PalpitesForm({ matches }: { matches: MatchData[] }) {
  ...
  const visible = useMemo(() => {
    ...
    return matches.filter((m) => {
      ...
      const isOpen = new Date(m.kickoff).getTime() > now;
...
function PalpiteCard({ match: m }: { match: MatchData }) {
  ...
  const live = isLive(kickoff, m.finished);

  // estado dos 4 mercados
  const [h, setH] = useState(m.palpite?.homeScore !== null && m.palpite?.homeScore !== undefined ? String(m.palpite.homeScore) : "");
  const [a, setA] = useState(m.palpite?.awayScore !== null && m.palpite?.awayScore !== undefined ? String(m.palpite.awayScore) : "");
  const [winner, setWinner] = useState<string>(m.palpite?.winnerGuess || "");
  const [totalGoals, setTotalGoals] = useState(
    m.palpite?.totalGoals !== null && m.palpite?.totalGoals !== undefined
      ? String(m.palpite.totalGoals)
      : "",
  );
  const [totalFouls, setTotalFouls] = useState(
    m.palpite?.totalFouls !== null && m.palpite?.totalFouls !== undefined
      ? String(m.palpite.totalFouls)
      : "",
  );
  ...
  async function save() {
    ...
    if (h !== "" && a !== "") {
      payload.homeScore = parseInt(h, 10);
      payload.awayScore = parseInt(a, 10);
    } else {
  ...
  const showResult = m.finished && m.homeScore !== null && m.awayScore !== null;
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: teamGradient,
        backgroundBlendMode: "screen",
      }}
    >
      {/* faixas laterais com cor do time */}
      <span
      ...
      <div className="flex items-center justify-between mb-3 gap-2 flex-wrap relative">
        ...
        <div className="flex items-center gap-2">
          ...
          {locked && !live && !showResult && (
            <span className="text-[0.7rem] uppercase tracking-widest text-amber-300">
              🔒 Fechado
            </span>
          )}
          {showResult && (
          ...
          <span className="text-[0.7rem] text-zinc-500 uppercase tracking-wider">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 mb-4 relative">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[0.65rem] uppercase tracking-widest text-zinc-500">
              mandante
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            style={{ borderColor: h !== "" ? home.color : undefined }}
          />
          ...
          <input
            ...
            placeholder="-"
            className="score-input"
            style={{ borderColor: a !== "" ? away.color : undefined }}
          />
        </div>
      ...
      <div className="mb-3 relative">
        <div className="text-[0.65rem] uppercase tracking-widest text-zinc-500 mb-1.5">
          Quem vence? (palpite sem placar)
        </div>
        <div className="grid grid-cols-3 gap-2">
          <OddBtn
            ...
            color={home.color}
            disabled={locked}
            onClick={() => setWinner(winner === "HOME" ? "" : "HOME")}
          />
          <OddBtn
            label="Empate"
            sub="🟰"
            ...
            color="#a1a1aa"
            disabled={locked}
            onClick={() => setWinner(winner === "DRAW" ? "" : "DRAW")}
          />
          <OddBtn
            label="Visitante"
            sub={away.flag}
            ...
            color={away.color}
            disabled={locked}
            onClick={() => setWinner(winner === "AWAY" ? "" : "AWAY")}
          />
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-3 mb-3 relative">
        ...
        <NumberMarket
          ...
          value={totalFouls}
          disabled={locked}
          onChange={setTotalFouls}
        />
      </div>

      {/* footer com palpite atual + pontos por mercado */}
        ...
        <div className="text-xs text-zinc-400 flex items-center gap-2 flex-wrap">
          {m.palpite ? (
            <>
              ...
              <SummaryChips p={m.palpite} showResult={showResult} />
            </>
          ) : locked ? (
        ...
        {!locked && (
          <div className="flex items-center gap-2">
            {status && (
              <span
                className={
                  "text-xs " + (status.kind === "ok" ? "text-emerald-400" : "text-red-400")
                }
              >
                {status.msg}
              </span>
...
function OddBtn({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={
        "rounded-lg border px-3 py-2 text-center transition-all relative overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
      }
      style={{
        borderColor: active ? color : "var(--c-border)",
        background: active ? `${color}1a` : "var(--c-bg-soft)",
        boxShadow: active ? `0 0 0 1px ${color}80 inset` : undefined,
      }}
    >
      <div className="text-2xl leading-none">{sub}</div>
      <div className="text-[0.7rem] uppercase tracking-wider font-semibold mt-1 text-zinc-200">
        {label}
      </div>
    </button>
  );
  ...
  return (
    <div className="bg-[color:var(--c-bg-soft)] border border-[color:var(--c-border)] rounded-lg p-2.5 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[0.65rem] uppercase tracking-widest text-zinc-500 leading-tight">
          {label}
        </div>
      ...
      <input
        ...
        placeholder="-"
        className="score-input h-12 w-14 text-xl"
      />
    </div>
  );
}
...
function SummaryChips({ p, showResult }: { p: Palpite; showResult: boolean }) {
  ...
  if (p.homeScore !== null && p.awayScore !== null) {
    chips.push({
      label: "Placar",
      value: `${p.homeScore} × ${p.awayScore}`,
      pts: showResult ? p.scorePoints : undefined,
    });
  }
  if (p.winnerGuess) {
    const v =
      p.winnerGuess === "HOME" ? "Mandante" : p.winnerGuess === "AWAY" ? "Visitante" : "Empate";
    chips.push({ label: "Vencedor", value: v, pts: showResult ? p.winnerPoints : undefined });
  }
  if (p.totalGoals !== null) {
    chips.push({
      label: "Gols",
      value: String(p.totalGoals),
      pts: showResult ? p.goalsPoints : undefined,
    });
  }
  if (p.totalFouls !== null) {
    chips.push({
      label: "Faltas",
      value: String(p.totalFouls),
      pts: showResult ? p.foulsPoints : undefined,
    });
  }
  if (chips.length === 0) {
    return <span className="text-zinc-600">Sem palpites preenchidos</span>;
  }
  return (
    <>
      {chips.map((c) => (
        <span key={c.label} className="chip">
          ...
          {typeof c.pts === "number" && (
            <span
              className={
                c.pts > 0 ? "text-emerald-400 font-display" : "text-zinc-500 font-display"
              }
            >
              {c.pts > 0 ? `+${c.pts}` : "0"}
            </span>
